...
type IranLogoVariation = "default" | "flip";
type IranLogoSize = "nav" | "footer";

type IranLogoProps = {
  locale: Locale;
  compact?: boolean;
  variation?: IranLogoVariation;
};
...
const widthBySize: Record<IranLogoSize, string> = {
  nav: "124px",
  footer: "148px",
};

const frameWidthBySize: Record<IranLogoSize, string> = {
  nav: "144px",
  footer: "168px",
};

const paddingXBySize: Record<IranLogoSize, string> = {
  nav: "10px",
  footer: "12px",
};

const paddingYBySize: Record<IranLogoSize, string> = {
  nav: "8px",
  footer: "9px",
};
...
export default function IranLogo({
  compact,
  variation = "default",
}: IranLogoProps) {
  const size: IranLogoSize = compact ? "nav" : "footer";

  return (
    <span
      aria-label="Melbet"
      className={cn("iran-logo", variation === "flip" && "iran-logo--flip")}
      role="img"
      style={
        {
          "--iran-logo-width": widthBySize[size],
          "--iran-logo-frame-width": frameWidthBySize[size],
          "--iran-logo-pad-x": paddingXBySize[size],
          "--iran-logo-pad-y": paddingYBySize[size],
        } as CSSProperties
      }
    >
      <svg
        aria-hidden="true"
        className="iran-logo__svg"
        focusable="false"
      ...
      >
        {letters.map((letter, index) => (
          <g
            className={cn(
              "iran-logo__letter",
              `iran-logo__letter--${letter.classSuffix}`,
              `iran-logo__letter--${letter.tone}`,
            )}
            data-letter={letter.dataLetter}
            key={letter.dataLetter}
            style={
              {
                "--iran-logo-letter-delay": `${index * 110}ms`,
                "--iran-logo-letter-index": index,
              } as CSSProperties
            }
          >
            <path d={letter.path} fill={toneFill[letter.tone]} />
          </g>
        ))}
      </svg>
    </span>
